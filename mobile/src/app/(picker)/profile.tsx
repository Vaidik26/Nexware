    ...
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      ...
      <View className="px-4 py-3.5 bg-white mb-4 border-b border-gray-100 shadow-sm flex-row items-center justify-between">
        <View className="flex-row items-center">
          ...
          <Text className="text-xl font-bold text-[#003527] font-inter">Operative Profile</Text>
        </View>
        <View className={`px-2.5 py-1 rounded-full flex-row items-center border ${
          ...
          <Text className={`text-[10px] font-extrabold font-inter uppercase tracking-wider ${isOnline ? 'text-[#006c49]' : 'text-slate-600'}`}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 32 }} className="flex-1">
        ...
        <View className="bg-white rounded-3xl p-6 items-center border border-gray-200 shadow-sm mb-5 relative overflow-hidden">
          <View className="w-20 h-20 bg-[#003527] border-2 border-emerald-400 rounded-full items-center justify-center mb-3.5 shadow-md">
            <Text className="text-2xl font-extrabold text-white font-inter">{picker?.initials || fullName.substring(0, 2).toUpperCase()}</Text>
          </View>
          <Text className="text-xl font-extrabold text-onSurface font-inter text-center">{fullName}</Text>
          ...
          <View className="bg-[#ecfdf5] border border-[#a7f3d0] px-3.5 py-1 rounded-full flex-row items-center mt-2 mb-4">
            ...
            <Text className="text-xs font-bold text-[#065f46] font-inter ml-1.5 uppercase tracking-wide">Picker (Floor Operative)</Text>
          </View>
          ...
          <View className="w-full bg-gray-50 rounded-2xl p-4 space-y-3 border border-gray-100 mt-1">
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                <Text className="text-xs font-semibold text-gray-500 font-inter">Username</Text>
              </View>
              <Text className="text-xs font-bold text-gray-800 font-inter">@{userName}</Text>
            </View>
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                <Text className="text-xs font-semibold text-gray-500 font-inter">Email Address</Text>
              </View>
              <Text className="text-xs font-bold text-gray-800 font-inter">{emailAddr}</Text>
            </View>
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                <Text className="text-xs font-semibold text-gray-500 font-inter">Security Clearance</Text>
              </View>
              <Text className="text-xs font-bold text-emerald-700 font-inter">Touch-Tick Certified</Text>
            </View>
          </View>
        </View>
          ...
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center flex-1 pr-3">
              ...
              <View className="flex-1">
                <Text className="text-base font-extrabold text-gray-900 font-inter">
                  {isOnline ? 'Online (Available)' : 'Not Online (Offline)'}
                </Text>
                <Text className="text-xs text-gray-500 font-inter mt-0.5 leading-snug">
                  {isOnline 
        ...
        <View className="bg-white rounded-2xl border border-gray-200 shadow-sm mb-6 overflow-hidden">
          ...
          >
            ...
            <Text className="text-base font-bold text-red-600 font-inter">Sign Out of Terminal</Text>
          </TouchableOpacity>
        </View>

        {/* Brand Footer */}
        <View className="pb-6 items-center justify-center">
          <Text className="text-[11px] font-bold text-emerald-800/80 font-inter">
            NexWare Enterprise Suite • Floor Operational Module
          </Text>
          <Text className="text-[10px] text-gray-400 font-inter mt-0.5">
