   ...
   >
    ...
    <View className="flex-1 justify-center">
     <View className="flex-row items-center justify-between mb-1">
      <Text className={` text-sm ${!item.read ? 'font-extrabold text-[#0b1c30]' : 'font-semibold text-gray-700'}`}>{item.title}</Text>
      {!item.read && (
       <View className="bg-[#003527] px-2 py-0.5 rounded-full border border-emerald-400/30">
        <Text className="text-[9px] text-emerald-300 font-extrabold tracking-wider uppercase">NEW</Text>
       </View>
      )}
     </View>
     <Text className={`text-xs ${isCancelled ? 'text-red-900 font-medium' : 'text-gray-600'} leading-4`}>{item.message}</Text>
     <View className="flex-row items-center justify-between mt-2 pt-2 border-t border-gray-100">
      <Text className="text-[11px] text-gray-400 ">{item.time}</Text>
      <Text className={`text-[11px] ${isCancelled ? 'text-red-700' : 'text-[#006c49]'} font-bold`}>
  ...
  <SafeAreaView className="flex-1 bg-background" edges={['top']}>
   ...
   <View className="px-4 py-3.5 bg-white mb-3.5 border-b border-gray-100 shadow-sm flex-row items-center justify-between">
    <View className="flex-row items-center">
     ...
     <Text className="text-xl font-extrabold text-[#003527] ">Floor Notifications</Text>
    </View>
    <View className="bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full flex-row items-center">
     ...
     <Text className="text-[11px] font-extrabold text-[#006c49] ">4.5s Bell Ready</Text>
    </View>
   </View>
   ...
   <View className="mx-4 mb-4 bg-[#001712] rounded-[24px] p-5 border border-emerald-500/30 shadow-lg">
    <View className="flex-row items-center justify-between mb-2">
     <View className="flex-row items-center">
      <Text className="text-white font-extrabold text-base mr-2">🔔 Warehouse Alert Engine</Text>
     </View>
     {isRinging ? (
      <View className="bg-red-500 px-2.5 py-0.5 rounded-full border border-red-300">
       <Text className="text-white text-[10px] font-extrabold tracking-wider uppercase">🔊 RINGING (4.5S)...</Text>
      </View>
     ) : (
      <View className="bg-emerald-500/20 px-2 py-0.5 rounded-full border border-emerald-400/30">
       <Text className="text-emerald-300 text-[10px] font-bold ">TESTER</Text>
      </View>
     )}
    </View>
    <Text className="text-emerald-100/80 text-xs leading-4 mb-4">
   ...
   <FlatList
    ...
    ListEmptyComponent={
     <View className="items-center justify-center py-12">
      <Text className="text-gray-500 text-sm">No recent alerts or notifications.</Text>
     </View>
    }
   />
  </SafeAreaView>
